...
class TTS:
    ...
    def _get_ref_spec(self, ref_audio_path):
        try:
            audio_np, raw_sr = sf.read(ref_audio_path, dtype="float32", always_2d=True)
            audio_np = audio_np.T
        except Exception:
            audio_np, raw_sr = librosa.load(ref_audio_path, sr=None, mono=False)
            if audio_np.ndim == 1:
                audio_np = audio_np[np.newaxis, :]
        raw_audio = torch.from_numpy(np.ascontiguousarray(audio_np))
